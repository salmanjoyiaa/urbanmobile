...
async function checkAdmin() {
    ...
    if (!user) {
        return { supabase, isAdmin: false, error: "Unauthorized", status: 401 };
    }
    ...
    if (profile?.role !== "admin") {
        return { supabase, isAdmin: false, error: "Unauthorized: Admin access required", status: 403 };
    }

    return { supabase, isAdmin: true, error: null, status: 200 };
}
