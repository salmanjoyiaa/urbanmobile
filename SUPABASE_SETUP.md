...
Copy the entire content below, paste into SQL Editor, and click **Run**.
...
1. **Table Editor**: Verify 8 tables exist
2. **Storage**: Verify 4 buckets exist with public/private settings
3. **Database** → **Replication**: Enable Realtime on `visit_requests` and `notifications`
4. **Test Connection**: Run this query in SQL Editor:
   ```sql
   SELECT COUNT(*) FROM profiles;
   -- Should return 0 (empty table, ready for signups)
   ```

## 🔐 Create First Admin User

After first user signup via app:
```sql
-- Get user ID
